...
public class TestIntBinarySearch
{
	...
	public void TestSimple()
	{
		...
		assertEquals(-1, IntBinarySearch.find(dup, -6));
		assertEquals(0, IntBinarySearch.find(dup, -5));
		assertEquals(-2, IntBinarySearch.find(dup, -4));
		assertEquals(0, IntBinarySearch.find(dup, -3));
		assertEquals(-3, IntBinarySearch.find(dup, -2));
		assertEquals(-3, IntBinarySearch.find(dup, -1));
		assertEquals(0, IntBinarySearch.find(dup, 0));
		assertEquals(-4, IntBinarySearch.find(dup, 1));
		assertEquals(0, IntBinarySearch.find(dup, 2));
		assertEquals(-5, IntBinarySearch.find(dup, 3));
		assertEquals(0, IntBinarySearch.find(dup, 4));
		assertEquals(-6, IntBinarySearch.find(dup, 5));
	}	
